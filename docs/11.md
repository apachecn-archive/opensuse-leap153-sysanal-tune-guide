...
> 原文:[https://doc . opensuse . org/documentation/leap/tuning/html/book-tuning/cha-tuning-oprofile . html](https://doc.opensuse.org/documentation/leap/tuning/html/book-tuning/cha-tuning-oprofile.html)
...
[7.1 Conceptual overview](cha-tuning-oprofile.html#sec-tuning-oprofile-overview)

[7.2 Installation and requirements](cha-tuning-oprofile.html#sec-tuning-oprofile-install)

[7.3 Available OProfile utilities](cha-tuning-oprofile.html#sec-tuning-oprofile-utils)

[7.4 Using OProfile](cha-tuning-oprofile.html#sec-tuning-oprofile-using)

[7.5 Generating reports](cha-tuning-oprofile.html#sec-tuning-oprofiles-report)

[7.6 More information](cha-tuning-oprofile.html#sec-tuning-oprofile-moreinfo)
...
2.  决定是否要使用 Linux 内核进行概要分析:
    ...
    1.  Linux 内核的概要文件。执行以下命令，因为`operf`只能处理未压缩的图像:
    ...
    2.  没有 Linux 内核的概要文件。使用以下命令:
        ...
        要查看输出中哪些函数调用其他函数，另外使用`--callgraph`选项并设置最大*深度*:
...
在生成报告之前，确保`operf`已经停止。除非你用`--session-dir`提供了一个输出目录，`operf`已经把它的数据写到*CUR _ DIR*/oprofile _ data/samples/current，默认情况下报表工具`opreport`和`opannotate`会在那里查找。
